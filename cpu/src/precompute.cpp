...
PrecomputedScalarOptimized precompute_scalar_optimized(const Scalar& K, unsigned window_bits) {
    if (window_bits < 4 || window_bits > 7) {
        throw std::invalid_argument("window_bits must be between 4 and 7");
    }
    ...
    // Step 2: Generate wNAF digits
    std::vector<int32_t> wnaf1 = compute_wnaf(decomp.k1, window_bits);
    std::vector<int32_t> wnaf2 = compute_wnaf(decomp.k2, window_bits);
    
    // Step 3: RLE compression - merge both wNAF streams
    // Process from MSB to LSB (reverse order)
    int max_len = std::max(wnaf1.size(), wnaf2.size());
    result.steps.reserve(130);  // Typical: ~100-130 steps
    
    for (int i = max_len - 1; i >= 0; --i) {
        // Get digits (0 if out of range)
        int32_t d1 = (i < static_cast<int>(wnaf1.size())) ? wnaf1[i] : 0;
        int32_t d2 = (i < static_cast<int>(wnaf2.size())) ? wnaf2[i] : 0;
        
        // Always process this bit position
        PrecomputedScalarOptimized::Step step;
        step.num_doubles = 1;  // Every iteration does at least 1 double
        
        // Process d1 (k₁ / Q table)
        if (d1 != 0) {
            bool is_negative = (d1 < 0);
            unsigned abs_digit = is_negative ? static_cast<unsigned>(-d1) : static_cast<unsigned>(d1);
            unsigned index = (abs_digit - 1) / 2;
            
            step.idx1 = static_cast<uint8_t>(index);
            step.neg1 = is_negative;
        }
        // else idx1 stays 0xFF (skip)
        
        // Process d2 (k₂ / ψ(Q) table)
        if (d2 != 0) {
            bool is_negative = (d2 < 0);
            unsigned abs_digit = is_negative ? static_cast<unsigned>(-d2) : static_cast<unsigned>(d2);
            unsigned index = (abs_digit - 1) / 2;
            
            step.idx2 = static_cast<uint8_t>(index);
            step.neg2 = is_negative;
        }
        // else idx2 stays 0xFF (skip)
        
        result.steps.push_back(step);
    }
    
    // NOW apply RLE compression: merge consecutive "double-only" steps
    if (!result.steps.empty()) {
        std::vector<PrecomputedScalarOptimized::Step> compressed;
        compressed.reserve(result.steps.size());
        
        PrecomputedScalarOptimized::Step current = result.steps[0];
        
        for (size_t i = 1; i < result.steps.size(); ++i) {
            const auto& next = result.steps[i];
            
            // Can we merge? (both have no additions)
            if (current.idx1 == 0xFF && current.idx2 == 0xFF && 
                next.idx1 == 0xFF && next.idx2 == 0xFF) {
                // Merge: accumulate doubles
                current.num_doubles += next.num_doubles;
            } else {
        ...
        result.steps = std::move(compressed);
    }
...
Point scalar_mul_arbitrary_precomputed_optimized(const Point& Q, const PrecomputedScalarOptimized& precomp) {
    ...
    unsigned table_size = (1u << (window_bits - 1));  // 2^(w-1) entries
    
    // Step 1: Compute ψ(Q) using endomorphism
    // ψ(x, y) = (β·x, y) where β³ ≡ 1 (mod p)
    const FieldElement beta = const_beta();
    ...
    // Compute ψ(Q) = (β*X : Y : Z) in Jacobian
    JacobianPoint psi_Q = Q_jac;
    psi_Q.x = Q_jac.x * beta;  // Only 1 field multiplication!
    ...
    // Step 2: Build precomputed tables (affine coordinates)
    std::vector<JacobianPoint> table_Q(table_size);
    std::vector<JacobianPoint> table_psi(table_size);
    ...
    // Fill tables: [Q, 3Q, 5Q, ..., (2^w-1)Q]
    for (unsigned i = 1; i < table_size; ++i) {
        table_Q[i] = jacobian_add(table_Q[i-1], Q2);
        table_psi[i] = jacobian_add(table_psi[i-1], psi2);
    }
    ...
    // Step 3: Main loop with RLE - MUCH FEWER ITERATIONS!
    // Each step: N doubles + 0-2 additions
    JacobianPoint result = {FieldElement::zero(), FieldElement::one(), FieldElement::zero(), true};
    
    for (const auto& step : precomp.steps) {
        // Batch doubles (RLE compression!)
        for (uint16_t d = 0; d < step.num_doubles; ++d) {
            result = jacobian_double(result);
        }
        
        // Addition from k₁ (Q table)
        if (step.idx1 != 0xFF) {
