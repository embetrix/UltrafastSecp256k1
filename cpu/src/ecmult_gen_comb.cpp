...
void CombGenContext::build_table() {
    std::size_t table_entries = static_cast<std::size_t>(1) << teeth_;
    ...
    for (std::size_t i = 1; i < table_entries; ++i) {
        ...
        std::size_t prev = i ^ (static_cast<std::size_t>(1) << lsb);
        Point sum = (prev == 0) ? bases[lsb]
...
Point CombGenContext::mul(const Scalar& k) const {
    ...
    for (int b = static_cast<int>(spacing_) - 1; b >= 0; --b) {
        ...
        uint32_t idx = extract_comb_index(k, static_cast<unsigned>(b));
        ...
        Point P = Point::from_jacobian_coords(entry.x, entry.y, FieldElement::one(), false);
        R = R.add(P);
    }
...
Point CombGenContext::mul_ct(const Scalar& k) const {
    ...
    for (int b = static_cast<int>(spacing_) - 1; b >= 0; --b) {
        ...
        for (std::size_t i = 0; i < table_entries; ++i) {
            uint64_t mask = ct::eq_mask(static_cast<uint64_t>(i),
            ...
            uint64_t inf_val = table_[i].infinity ? UINT64_MAX : 0;
...
bool CombGenContext::save_cache(const std::string& path) const {
    ...
    uint32_t count = static_cast<uint32_t>(table_.size());
...
void init_comb_gen(unsigned teeth) {
    std::lock_guard<std::mutex> lock(g_comb_mutex);
    if (!g_comb_ctx.ready()) {
