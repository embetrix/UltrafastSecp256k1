...
namespace glv_constants {
    // λ (lambda) - eigenvalue where λ·G = φ(G)
    ...
    // These are used to split k → (k1, k2) such that k = k1 + k2·λ (mod n)
...
// Apply endomorphism to point: φ(x,y) = (β·x, y)
...
// Should verify: φ(φ(P)) + P = O (point at infinity)
