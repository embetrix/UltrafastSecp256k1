...
bool fe26_equals_fe64(const FieldElement26& a26, const FieldElement& a64) {
    FieldElement converted = a26.to_fe();
...
void test_addition() {
    ...
    for (int i = 0; i < NUM_VECTORS; ++i) {
        for (int j = 0; j < NUM_VECTORS; ++j) {
            ...
            FieldElement26 b26 = FieldElement26::from_fe(b);
            FieldElement26 sum26 = a26 + b26;

            bool ok = fe26_equals_fe64(sum26, sum64);
            if (!ok) {
                char buf[128];
...
void test_negate() {
    ...
    for (int i = 0; i < NUM_VECTORS; ++i) {
        ...
        FieldElement26 fe26 = FieldElement26::from_fe(fe);
        FieldElement26 neg26 = fe26.negate(1);

        bool ok = fe26_equals_fe64(neg26, neg64);
        if (!ok) {
            char buf[128];
...
void test_multiplication() {
    ...
    for (int i = 0; i < NUM_VECTORS; ++i) {
        for (int j = 0; j < NUM_VECTORS; ++j) {
            ...
            FieldElement26 a26 = FieldElement26::from_fe(a);
            FieldElement26 b26 = FieldElement26::from_fe(b);
            FieldElement26 prod26 = a26 * b26;

            bool ok = fe26_equals_fe64(prod26, prod64);
            if (!ok) {
                char buf[128];
...
void test_squaring() {
    ...
    for (int i = 0; i < NUM_VECTORS; ++i) {
        ...
        FieldElement26 a26 = FieldElement26::from_fe(a);
        FieldElement26 sq26 = a26.square();

        bool ok = fe26_equals_fe64(sq26, sq64);
        if (!ok) {
            char buf[128];
...
void test_mixed_operations() {
    ...
    FieldElement26 bsq26 = b26.square();
    r26 = r26 + bsq26;
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        FieldElement26 tsq26 = t26.square();

        FieldElement bt64 = b * t64;
        FieldElement26 bt26 = b26 * t26;
...
void test_half() {
    ...
    for (int i = 0; i < NUM_VECTORS; ++i) {
        ...
        FieldElement26 h = a26.half();

        // Verify: h + h == a
        FieldElement26 dbl = h + h;
...
void test_normalize_edge() {
    ...
    pm1.normalize();
    FieldElement pm1_64 = FieldElement::from_limbs(VECTORS[6].limbs);  // p-1
...
void test_commutativity_associativity() {
    ...
    FieldElement26 ab = a + b;
    FieldElement26 ba = b + a;
    CHECK(ab == ba, "a+b == b+a");

    // Commutative multiplication
    FieldElement26 a_times_b = a * b;
    FieldElement26 b_times_a = b * a;
    CHECK(a_times_b == b_times_a, "a*b == b*a");

    // Associative multiplication
    FieldElement26 ab_c = (a * b) * c;
    FieldElement26 a_bc = a * (b * c);
    CHECK(ab_c == a_bc, "(a*b)*c == a*(b*c)");

    // Distributive: a * (b + c) == a*b + a*c
    FieldElement26 bc_sum = b + c;
    bc_sum.normalize_weak();
    FieldElement26 dist_left = a * bc_sum;
    FieldElement26 dist_right = (a * b) + (a * c);
...
void test_mul_after_lazy_add() {
    ...
    FieldElement b = FieldElement::from_limbs(VECTORS[8].limbs);
    ...
    FieldElement prod64 = sum64 * b;
    FieldElement26 prod26 = sum26 * b26;
    ...
    FieldElement prod2_64 = sum64 * sum2_64;
    FieldElement26 prod2_26 = sum26 * sum2_26;
